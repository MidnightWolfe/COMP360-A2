...
##Positioning variables for the image as well as the path
var pathXOffset = 15
var pathYOffset = 1.8
var pathZOffset = 10
...
func _CardinalSpline(projectedPoints):
...
func _CatmullRom():
	...
	for k in range(points.size()-3):
		...
		for i in range(splinePoints.size()-1):
			_BresenhamLine(splinePoints[i].x, splinePoints[i].y, splinePoints[i+1].x, splinePoints[i+1].y )
	...
	_CreateImageInWorld(splineSprite)
	pass
...
func _BresenhamLine(funcX0, funcY0, funcX1, funcY1):
	var x1 = floor(funcX0)
	var x2 = floor(funcX1)
	var y1 = floor(funcY0)
	var y2 = floor(funcY1)
...
func _CreateImageInWorld(image: Sprite2D):
...
func _DebugShowPath(splineCurve: Curve3D):
